...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($login_identifier) || empty($password)) {
        ...
        try {
            ...
            $sql = "SELECT doctor_id, username, email, password, theme, profile_picture FROM doctors WHERE $field = ?";
            ...
            if ($user = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                if (password_verify($password, $user["password"])) {
                    ...
                    $update_sql = "UPDATE doctors SET last_login = CURRENT_TIMESTAMP WHERE doctor_id = ?";
                    $update_stmt = $pdo->prepare($update_sql);
                    $update_stmt->execute([$user["doctor_id"]]);
...
<head>
    ...
    <meta name="description" content="Doctor login portal for HeartCare Connect">
    <title>Doctor Login - HeartCare Connect</title>
...
<body>
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="login_identifier" name="login_identifier" 
                           value="<?php echo htmlspecialchars($login_identifier); ?>" required autofocus>
    ...
    <script>
        ...
        function createParticles() {
            ...
            const colors = ['#ff4757', '#ff6b81', '#ff8a9d', '#ffa5b9'];
